...
class Studente{
	...
	int compareTo(const Studente& s) {
		...
		if(this->cognome > s.getCognome()) {
			...
			if(this->nome > s.getNome()){
			    ...
			    if(this->matricola > s.getMatricola()){
				return GREATER;
		    	}
	...
	public:
		...
		Studente(string cognome, string nome, string matricola, int eta): cognome(cognome), nome(nome), matricola(matricola), eta(eta){}
